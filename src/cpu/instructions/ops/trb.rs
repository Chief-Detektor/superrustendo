use crate::{
    cpu::{addressmodes::AddressModes, instructions::Operant_Value, address::Address},
    mem::Bus,
};

pub fn trb(bus: &mut Bus, address: Address, address_mode: AddressModes) /* -> usize */
{
...
mod tests{
    ...
    fn trb_instruction_8_bit() {
        ...
        assert!(bus.read(Address::new(0x0)) == 0b1010101);
        assert!(bus.get_cpu().get_regs().get_P().get_z() == 0);
    }
    #[test]
    fn trb_instruction_16_bit_zero(){
        let mut bus = Bus::new();
        let mut P = StatusRegister::default();
        P.set_m(0);
        P.set_z(0);
    ...
    fn trb_instruction_16_bit() {
        ...
        super::trb(&mut bus, address, address_mode);
        println!("P: {:?}", bus.get_cpu().get_regs().get_P());
        assert!(bus.read(Address::new(0x0)) == 0b101);
        assert!(bus.read(Address::new(0x1)) == 0b1010101);
        assert!(bus.get_cpu().get_regs().get_P().get_z() == 0);
